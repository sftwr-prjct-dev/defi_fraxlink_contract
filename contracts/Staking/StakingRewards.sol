...
contract StakingRewards is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard, Pausable {
    ...
    uint256 public rewardRate = (uint256(51000000e18)).div(10 * (365 * 86400)); // 51M XESH over 10 years
    ...
    constructor(
        ...
        XEB = XEBECStablecoin(_xeb_address);
    ...
    function crBoostMultiplier() public view returns (uint256) {
        uint256 boost_to_subtract = cr_boost_max_multiplier.mul(XEB.global_collateral_ratio()).div(1e6);
    ...
    function retroCatchUp() internal {
        ...
        require(rewardRate.mul(rewardsDuration).mul(crBoostMultiplier()).mul(num_periods_elapsed + 1).div(1e6) <= balance, "Not enough XESH available for rewards!");
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount) external onlyOwner {
        // If it's XEB we have to query the token symbol to ensure its not a proxy or underlying
        bool isXEB = (keccak256(bytes("XEB")) == keccak256(bytes(ERC20(tokenAddress).symbol())));
        // Cannot recover the staking token or the rewards token
        require(
            tokenAddress != address(stakingToken) && tokenAddress != address(rewardsToken) && !isXEB,
